...
const sendApplicationReceived = async (email, name,
  jobTitle, company) => {
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#1B4D3E;padding:24px;text-align:center;">
      ...
      <div style="padding:32px 24px;">
        ...
        <p style="color:#4B5563;font-size:16px;">
          ...
          at <strong>${company}</strong> has been submitted
          successfully.
        </p>
        <p style="color:#4B5563;font-size:16px;">
  ...
  return sendMail(email,
    `Application Confirmed — ${jobTitle}`, html);
};
...
const sendInterviewScheduledEmail = async (email, name,
      ...
      <div style="padding:32px 24px;">
        ...
        <div style="background:#FEF2F2;padding:12px;
          ...
          <p style="margin:0;color:#DC2626;font-size:14px;">
            If you fail to attend, your Challenge Fee
            will be forfeited.
          </p>
        </div>
        <div style="text-align:center;margin:24px 0;">
  ...
  `;
  return sendMail(email,
    `Interview Scheduled — ${jobTitle}`, html);
};

// ---- SHORTLISTED ----

const sendShortlistedEmail = async (email, name,
  jobTitle, company) => {
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#1B4D3E;padding:24px;text-align:center;">
  ...
  `;
  return sendMail(email,
    `Great News! Shortlisted for ${jobTitle}`, html);
};

// ---- REJECTED ----

const sendRejectedEmail = async (email, name, jobTitle,
  company, feeAmount, refundId) => {
  const refundSection = feeAmount > 0 && refundId ? `
    <div style="background:#FFFBEB;padding:16px;border-radius:8px;
      ...
      <p style="margin:0;color:#92400E;font-size:14px;">
        Your Rs.${feeAmount} Challenge Fee has been refunded.
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="padding:32px 24px;">
        ...
        <p style="color:#4B5563;font-size:16px;">
          ...
          <strong>${jobTitle}</strong> at
          <strong>${company}</strong>. The recruiter has
  ...
  `;
  return sendMail(email,
    `Update on Your Application — ${jobTitle}`, html);
};

// ---- REFUND ----

const sendRefundEmail = async (email, name, amount,
  reason) => {
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#1B4D3E;padding:24px;text-align:center;">
  ...
  `;
  return sendMail(email,
    `Challenge Fee Refunded — Rs.${amount}`, html);
};

// ---- HIRED ----

const sendHiredEmail = async (email, name, jobTitle,
  company) => {
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#1B4D3E;padding:24px;text-align:center;">
  ...
  `;
  return sendMail(email,
    `Congratulations! Hired — ${jobTitle}`, html);
};
      ...
      <div style="padding:32px 24px;">
        ...
        <div style="background:#FEF2F2;padding:16px;
          ...
          <p style="margin:0;color:#DC2626;font-size:16px;">
            Your Rs.${amount} Challenge Fee for
            <strong>${jobTitle}</strong> at
        ...
        <p style="color:#4B5563;font-size:14px;">
          Please attend scheduled interviews to avoid
          fee forfeiture in the future.
        </p>
      </div>
      <div style="background:#F9FAFB;padding:16px 24px;
        text-align:center;font-size:12px;color:#9CA3AF;">
...
const sendResetOTPEmail = async (email, name, otp) => {
  ...
  `;
  return sendMail(email, 'TrustHire — Reset Your Password',
    html);
};
