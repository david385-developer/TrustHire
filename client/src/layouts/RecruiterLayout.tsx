  ...
  const getBreadcrumbs = () => {
    ...
    return (
      <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium select-none">
        <Link to="/recruiter/dashboard" className="hover:text-indigo-600 dark:hover:text-indigo-400">Recruiter</Link>
        {paths.map((p, i) => {
          if (p === 'recruiter') return null;
          ...
          return (
            <React.Fragment key={i}>
              ...
              {isLast ? (
                ...
                <Link to={`/recruiter/${p}`} className="capitalize hover:text-indigo-600 dark:hover:text-indigo-400">{display}</Link>
              )}
            </React.Fragment>
          );
        })}
        ...
        {isCollapsed ? (
          ...
          <>
            ...
            <div className="p-4 border-b border-slate-900/50 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-white truncate">{user?.company || user?.name}</p>
                <span className="inline-block text-[10px] px-2 py-0.5 rounded bg-slate-800 text-indigo-400 font-bold uppercase tracking-wider mt-0.5">
                  Recruiter
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-1">
              <div className="hidden sm:block text-right">
                ...
                <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-1 font-bold">Recruiter</p>
              </div>
